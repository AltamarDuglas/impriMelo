...
const ConfigStep = ({
  ...
  // Filtrar modos disponibles según el tipo de papel (Mosaico solo para stickers)
  const availableModes = MODES.filter(m => {
    if (m.id === 'mosaico') return config.paperType === 'stickers';
    return true;
  });

  // Asegurar que el modo sea válido si el usuario cambia de papel
  React.useEffect(() => {
    if (config.printMode === 'mosaico' && config.paperType !== 'stickers') {
      setConfig(prev => ({ ...prev, printMode: 'individual' }));
    }
  }, [config.paperType, config.printMode, setConfig]);
  ...
  return (
    ...
    >
      {/* Vista Previa (Solo Escritorio) - Liberada de la tarjeta */}
      <div className="hidden lg:flex lg:order-2 h-full flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="order-2 lg:order-1 flex-1 lg:h-full overflow-y-auto space-y-6 pr-1 custom-scrollbar pb-32 lg:pb-10">
        ...
        <div className="flex flex-col divide-y divide-slate-200/50">
            ...
            <div className="grid grid-cols-2 gap-3">
              {PAPERS.map((p) => (
                <button
                  ...
                  className={`relative p-3 rounded-3xl border-2 text-left transition-all duration-500 overflow-hidden min-h-[130px] flex flex-col justify-end ${config.paperType === p.id
                    ? 'border-pink-500 shadow-lg scale-[1.02] grayscale-0 opacity-100'
                    : 'border-white/50 hover:border-pink-300 grayscale opacity-75 hover:grayscale-0 hover:opacity-100'
                    }`}
                >
              ...
              <div className="relative p-1 bg-white/50 backdrop-blur-xl rounded-[2rem] border border-white/60 flex items-center h-20 shadow-inner overflow-hidden">
                ...
                <div className="absolute inset-0 w-full h-full p-1 pointer-events-none">
                  <div 
                    ...
                    style={{ gridTemplateColumns: `repeat(${availableModes.length}, 1fr)` }}
                  >
                ...
                {availableModes.map((m) => (
                  ...
                  >
                    <m.Icon className={`w-6 h-6 transition-all duration-500 ${config.printMode === m.id ? 'text-pink-500 scale-110' : 'text-slate-400'
                      }`} />
                    <p className={`text-[10px] font-bold tracking-widest transition-colors ${config.printMode === m.id ? 'text-slate-950' : 'text-slate-500'
                      }`}>
                      {m.label}
                    </p>
                  </button>
          ...
          <div className="p-1 md:p-4 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-bold text-slate-800 tracking-widest">Vista Previa</p>
            </div>
            <div className="w-full lg:hidden relative overflow-hidden transition-all duration-500">
              <PagePreview
                config={config}
                previews={previews}
                pdfPagePreviews={pdfPagePreviews}
          ...
          {/* 4 y 5. Orientación y Copias */}
          <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-white/50">
            <div className="p-3 md:p-5 space-y-3">
              <div className="flex items-center gap-3">
                <span className="w-8 h-8 rounded-full bg-cyan-100 text-cyan-600 flex items-center justify-center text-xs font-black shadow-sm">4</span>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 p-3 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${config.orientation === 'portrait' ? 'border-pink-500 bg-pink-50 text-pink-600' : 'border-white/60 bg-white/50 text-slate-400'
                    }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 p-3 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${config.orientation === 'landscape' ? 'border-pink-500 bg-pink-50 text-pink-600' : 'border-white/60 bg-white/50 text-slate-400'
                    }`}
                >
            ...
            <div className="p-3 md:p-5 space-y-3">
              <div className="flex items-center gap-3">
                <span className="w-8 h-8 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center text-xs font-black shadow-sm">5</span>
        ...
        </div> {/* FIN DEL CONTENEDOR PRINCIPAL UNIFICADO */}
